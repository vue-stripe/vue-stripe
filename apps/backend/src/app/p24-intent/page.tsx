...
export default function P24IntentPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(data.error || 'Failed to create P24 Payment Intent')
      }
    ...
    <div className="mx-auto max-w-2xl">
      <div className="mb-6">
        <div className="flex items-center gap-3">
          <div className="flex h-10 w-10 items-center justify-center rounded-lg bg-blue-600">
            <span className="text-sm font-bold text-white">P24</span>
          </div>
          <div>
            <h1 className="text-2xl font-bold">Przelewy24 Payment Intent</h1>
            <p className="text-sm text-muted-foreground">
              Create a P24 Payment Intent for Polish bank payments
            </p>
          </div>
        </div>
      </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Configure the P24 payment. Supports EUR and PLN currencies. The client secret can be used with
      ...
      {result && (
        <div className="mt-6">
          <ResultDisplay
            title="P24 Payment Intent Created"
            fields={[
              {
                label: 'Client Secret',
