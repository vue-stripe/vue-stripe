...
export default function IdealIntentPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(data.error || 'Failed to create iDEAL Payment Intent')
      }
    ...
    <div className="mx-auto max-w-2xl">
      <div className="mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold">iDEAL Payment Intent</h1>
            <p className="text-sm text-muted-foreground">
              Create an iDEAL Payment Intent for Dutch bank payments
            </p>
          </div>
        </div>
      </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Configure the iDEAL payment. Currency is always EUR. The client secret can be used with
      ...
      {result && (
        <div className="mt-6">
          <ResultDisplay
            title="iDEAL Payment Intent Created"
            fields={[
              {
                label: 'Client Secret',
