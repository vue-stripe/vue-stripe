...
const canConfirm = computed(() => cardReady.value && !cardError.value && !confirming.value)
...
function onCardChange(event: any) {
...
<template>
  <div class="card-element-example">
            ...
            <VueStripeProvider :publishable-key="publishableKey">
              <VueStripeElements :client-secret="clientSecret!">
                <div class="card-element-wrapper">
                  <VueStripeCardElement @ready="onCardReady" @change="onCardChange" />
                </div>
                <div v-if="cardError" class="field-error">{{ cardError }}</div>
...
const ConfirmCardButton = defineComponent({
  name: 'ConfirmCardButton',
...
export default { components: { ConfirmCardButton } }
</script>

<style scoped>
.card-element-example { padding: 1.5rem; background: var(--vp-c-bg); border: 1px solid var(--vp-c-divider); border-radius: 12px; }
...
.card-element-wrapper { padding: 1rem; background: white; border: 1px solid var(--vp-c-divider); border-radius: 6px; margin-bottom: 1rem; }
:root.dark .card-element-wrapper { background: #1a1a1a; }
.field-error { margin-bottom: 1rem; padding: 0.5rem 0.75rem; background: var(--vp-c-danger-soft); border-radius: 4px; color: var(--vp-c-danger-1); font-size: 0.8125rem; }
