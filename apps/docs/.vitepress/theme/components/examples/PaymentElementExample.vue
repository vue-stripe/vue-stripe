...
const canConfirm = computed(() => elementsReady.value && paymentReady.value && !confirming.value)
...
<template>
  <div class="payment-element-example">
            ...
            <VueStripeProvider :publishable-key="publishableKey">
              <VueStripeElements :client-secret="clientSecret!" @ready="elementsReady = true">
                ...
                <div class="ready-indicators">
                  <span :class="['indicator', { ready: elementsReady }]">{{ elementsReady ? '✓' : '○' }} Elements</span>
...
const ConfirmPaymentButton = defineComponent({
...
export default { components: { ConfirmPaymentButton } }
</script>

<style scoped>
.payment-element-example { padding: 1.5rem; background: var(--vp-c-bg); border: 1px solid var(--vp-c-divider); border-radius: 12px; }
...
.ready-indicators { display: flex; gap: 1rem; margin: 1rem 0; padding: 0.5rem 0.75rem; background: var(--vp-c-bg-soft); border-radius: 6px; }
