...
var VueStripeCheckout = {
  install: function install(Vue, _key) {
    ...
    Vue.component('VueStripeCheckout', (_Vue$component = {
      data: function data() {
        return {
          token: undefined,
          args: undefined
      ...
      mounted: function mounted() {
        ...
        var scriptCheckout = document.createElement('script');
        ...
        var scriptStripeV2 = document.createElement('script');
        ...
        var scriptFeatherlight = document.createElement('script');
          ...
          var options = {
            ...
            token: function token(_token, args) {
              ...
              Stripe.source.create({
                type: 'card',
                token: _token.id
              }, _this.stripeCardResponseHandler);
            },
      ...
      stripeCardResponseHandler: function stripeCardResponseHandler(status, response) {
        if (response.error) {
        ...
        var returnURL = "https://shop.example.com/static_page"; //TODO superheri add this into props
      ...
      stripe3DSecureResponseHandler: function stripe3DSecureResponseHandler(status, response) {
        if (response.error) {
        ...
        Stripe.source.poll(response.id, response.client_secret, this.stripe3DSStatusChangedHandler);
      ...
      stripe3DSStatusChangedHandler: function stripe3DSStatusChangedHandler(status, source) {
        if (source.status == 'chargeable') {
          _jquery2.default.featherlight.current().close();
      ...
      displayResult: function displayResult(resultText) {
        alert(resultText);
